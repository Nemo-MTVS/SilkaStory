...
public class JDBCConnection {
    ...
    public static void printConnectionPoolStatus() {
        ...
        logger.info("[hikariCp 커넥션 풀 상태 ]");
        logger.info("총 커넥션 수 (total Connections) : {}", poolMXBean.getTotalConnections());
        logger.info("활성 커넥션 수 (Active Connections) : {}" , poolMXBean.getActiveConnections());
        logger.info("유휴 커넥션 수 (IDLE Connections) : {}" , poolMXBean.getIdleConnections());
        logger.info("대기 중인 커넥션 요청 수 (Pending Threads) : {}" , poolMXBean.getThreadsAwaitingConnection());
    }
}
